...
par(mfrow=c(2, 4), mar = c(2.5, 2.5, 2, 0.1), mgp = c(1.5, 0.3, 0))
for (i in 1:J) {
  beta_rep  <- samples[,paste0("beta[", i, "]")]
  ...
  plot(1:T, y[i,], type = "n", ylim = range(y), xlim = c(1, T), xlab = "time", ylab = "y",
       main = paste0("Individual ", i, " Est"))
  for (j in 1:20)
    lines(1:T, y_pred_rep[j,], col = "gray")
  lines(1:T, y[i,])
}
```
